...
compose-up-dev: ### Run docker compose with hot reload
	$(BASE_STACK) -f docker-compose.override.yml up -d --build -d && docker compose logs -f


compose-up: ### Run docker compose (without backend and reverse proxy)
	$(BASE_STACK) up --build -d && docker compose logs -f
.PHONY: compose-up
